...
createChannelTx2() {
	...
	set -x
	configtxgen -profile TwoOrgsChannel2 -outputCreateChannelTx ./channel-artifacts/${CHANNEL_NAME}.tx -channelID $CHANNEL_NAME
	res=$?
	{ set +x; } 2>/dev/null
	if [ $res -ne 0 ]; then
		fatalln "Failed to generate channel configuration transaction..."
...
createAnchorPeerTx2() {
	...
	infoln "Generating anchor peer update transaction for ${orgmsp}"
	set -x
	configtxgen -profile TwoOrgsChannel2 -outputAnchorPeersUpdate ./channel-artifacts/${CHANNEL_NAME}-${orgmsp}anchors.tx -channelID $CHANNEL_NAME -asOrg ${orgmsp}
	res=$?
	{ set +x; } 2>/dev/null
	if [ $res -ne 0 ]; then
		fatalln "Failed to generate anchor peer update transaction for ${orgmsp}..."
...
joinChannel() {
  ...
  setGlobals $ORG
	...
	# 로그만 안고쳤고 org4 로그가 나오면 peer1.org1 join한 것
}
...
## Join Org1, Org3 the peers to mychannel2
infoln "Join Org1 peers to mychannel2..."
joinChannel 1
infoln "Join Org3 peers to mychannel2..."
joinChannel 3

## Set the anchor peers for each org in the channel
infoln "Updating anchor peers for org1 on channel 'mychannel2'..."
updateAnchorPeers 1
